...
> Everything you need to run the [Course 2 cEOS lab](vxlan-evpn.md) on a Mac —
...
You're ready for the [Course 2 lab](vxlan-evpn.md). Drop in its `ceos-evpn.clab.yml`
and:
```bash
cd ~/ceos-lab
...
`destroy`/`deploy` as often as you like. Next: the [VXLAN-EVPN lab →](vxlan-evpn.md).
