#!/usr/bin/env bash
# ==============================================================================
# NetForge Labs — Automated Step Runner & Verifier (Phase 9 IPv6 Transition)
# ==============================================================================
set -euo pipefail
...
STEPS=(
  "01:IPv6 Neighbor Discovery (ND) & Link-Local Reachability"
)

preflight() {
  if ! docker ps --format '{{.Names}}' | grep -q "^${FABRIC}-r1-v6$"; then
...
apply_step() {
  ...
  for cfg in "${STEPS_DIR}/${num}"-*.cfg; do
    ...
    local node=$(echo "$fname" | cut -d- -f2-3 | sed 's/-nd//;s/-gnmi//')
...
run_guided_step() {
  ...
  for cfg in "${STEPS_DIR}/${num}"-*.cfg; do
    [ -f "$cfg" ] || continue
    local fname=$(basename "$cfg")
    local node=$(echo "$fname" | cut -d- -f2-3 | sed 's/-nd//;s/-v6//')
    echo "  ------------------------------------------------------------------------"
  ...
  echo "    docker exec -it ${FABRIC}-r1-v6 Cli -p 15 -c \"show ipv6 neighbors\""
  echo ""
...
list_steps() {
  echo "Available steps in ipv6-lab:"
...
case "$1" in
  ...
  --all|-a)
    ...
    echo ""
    echo "All IPv6 transition pipeline steps passed cleanly!"
    ;;
  *)
