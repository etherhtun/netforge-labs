#!/usr/bin/env bash
# ==============================================================================
# NetForge Labs — Automated Step Runner & Verifier (Segment Routing & Ti-LFA)
# ==============================================================================
set -euo pipefail
...
STEPS=(
  "01:IS-IS Segment Routing Node & Prefix SIDs"
  "02:Ti-LFA Sub-50ms Fast Reroute Protection"
  "03:SR-PCE & BGP Color Traffic Steering"
)

preflight() {
  if ! docker ps --format '{{.Names}}' | grep -q "^${FABRIC}-pe1$"; then
...
run_guided_step() {
  ...
  case "$num" in
    01) echo "    docker exec -it ${FABRIC}-pe1 Cli -p 15 -c \"show isis segment-routing prefix-segments\"" ;;
    02) echo "    docker exec -it ${FABRIC}-pe1 Cli -p 15 -c \"show ip route 10.255.0.5/32 detail\"" ;;
    03) echo "    docker exec -it ${FABRIC}-pe1 Cli -p 15 -c \"show traffic-engineering segment-routing policy\"" ;;
  esac
  echo ""
...
list_steps() {
  echo "Available steps in segment-routing-lab:"
